...
`CPU`将系统中的各类存储器看作一个逻辑存储器，给每个物理存储器划分了内存地址范围。`8086 CPU`的内存地址空间的分配情况:
...
偏移地址寄存器: `SP` `BP` `SI` `DI` `IP` `BX`(有时候作为偏移地址寄存器使用)几条汇编指令
...
- 在`8086PC`中，内存地址由段地址和偏移地址组成。其中段地址由`DS`寄存器指定。比如读取`10000H`单元的内容

- 长度就是依据寄存器的类型来确定的

```masm
mov bx,1000H
mov ds,bx      ; 不支持直接将数据送入段寄存器的操作，所以这里使用了 bx
mov al,[0]     ; 传送一个 字节
mov cx,[0]     ; 传送一个 字
mov [1],al
mov [1],cx
