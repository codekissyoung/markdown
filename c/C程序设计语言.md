...
`c = getchar();`从输入里获取一个字符,`putchar(c);`以字符形式输出一个字符/数字，`EOF`是文件结尾，`while( (c = getchar()) != EOF )` 判断文件结尾，`c == '\\n'` 判断字符相等，判断一行结束。
...
类型转换，允许字节小的类型自动转换为字节大的类型存储。C语言没有指定char类型的变量是`signed`还是`unsigned`。当一个char类型自动变为int类型时，其结果是否可能是负数？对于不同的机器，其结果也不相同。在某些机器中，如果char类型值的最左一位是`1`,则转为负整数。而在另一些机器中，则是在char类型值的左边添加0，这样导致转换的结果值总是正值。
...
表达式中没有`unsigned`操作数: 如果其中一个操作数为`long double`,则将另一个操作数转换为`long double`;其中一个操作为`double`,则另一个操作数也转换为`double`;其中一个操作为`float`,则另一个操作数也转换为`float`;`char`与`short`转换为`int`类型;其中一个操作数的类型为`long`,则将另一个操作数也转换为`long`类型。

表达式中含有`unsigned`操作数，转换规则会复杂一些。原因在于，`signed`与`unsigned`之间的比较运算是与机器相关的，它们取决于机器中不同整数类型的大小。例如，假设`int`类型占16位，`long`类型占32位，那么`-1L < 1U`,这是因为`unsigned int`将被提升为`signed long`类型。但`-1L > 1UL`,这是因为`-1L`会被提升为`unsigned long`类型，因而成为一个比较大的正数。
