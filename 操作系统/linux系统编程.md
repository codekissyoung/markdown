...
硬链接、软链接、文件、文件 I/O 模型、程序、进程、内存映射、静态库、动态库、进程间通信`IPC`、进程间同步、信号、线程、进程组、`shell`任务控制、会话、控制终端、控制进程、伪终端、日期和时间、`C/S`架构、实时性、`/proc`文件系统。
...
Libevent 是这样的一个软件层，它提供了检查文件描述符 IO 事件的抽象，
它底层能够识别并应用`select()`、`poll()`、`信号驱动IO`、`epoll`、`Solaris的/dev/pull`和`BSD的kqueue`接口。
