...
一个线程将无法操作共享数据直至`lck`的构造函数获取它的互斥锁 `(mutex) m`，最后将`lck`对应的析构函数负责释放资源。在上面的例子中，当控制线程离开`f()`时, (通过`return`语句跳转到函数末尾，或者因为抛出异常而离开函数)，`unique_lock` 的析构函数负责释放`(mutex) m`。
...
上述代码中，只要`shared_ptr`对象还剩一份拷贝存在，就不会实际销毁`fstream`对象。所以，除非你确实需要共享指针的所有权，否则别轻易使用`shared_ptr`。
...
函数仍要应用 C++ 函数调用和参数检查规则。

建议:

- 利用`C++11`新特性，来更新你的程序和编程技术，使之更现代化:
  - 使用构造函数建立不变式
  - 使用 构造/析构 函数简化资源管理
  - 避免 `裸new` 和 `裸delete`
  - 使用 容器 和 算法，而不是 内置数组 和 专用代码。
  - 优先使用标准库特性，而非自己开发的代码。
  - 使用异常而非错误代码，来报告不能局部处理的错误。
  - 使用移动语义来避免拷贝大对象
  - 使用`unique_ptr`来引用多态类型的对象
  - 使用`shared_ptr`来引用共享对象，即不止有一个所有者负责其析构的对象
  - 使用模板来保持静态类型安全（消除类型转换），并避免类层次结构的不必要使用
- 当你将一个C程序改写为C++程序时
  - 首先检查 函数声明 和 标准头文件 的使用是否一致
  - 重新命名与C++关键字同名的标识符
  - 将`malloc()`的结果转换为恰当的类型，或者索性将所有`malloc()`都改为`new`
  - 当使用`new`与`delete`替换`malloc`与`free`进行内存管理时，考虑使用`vector` `push_back()`和`reserve()`而非`realloc()`
- 出于兼容性和类型安全的考虑，如果必须使用C，应使用 C 与 C++ 的公共子集 编写程序
- C++ 不允许`int`到`enum`的隐式转换，如果必须进行这种转换，使用显示类型转换
- 每个C标准库的`<X.h>`都将名字定义在全局命名空间中，而对应的`<cX>`则将名字都定义在命名空间`std`中
- 声明C函数时，使用`extern "C"`
