...
指针 pointer：是 **指向（point to）另外一种类型的复合类型，它实际存储的是它所指向的对象的 内存地址。

指针本身就是一个对象，允许对指针赋值 和 拷贝，在指针的生命周期内，它可以先后指向几个不同的对象。

指针无须在定义时初始化。但未初始化的的指针，拥有一个不确定的值。
...
int *p;       // * 紧随着类型名出现，是声明的一部分，所以 r 是一个指针
p = &i;       // & 出现在表达式中，是一个取地址符
*p = i;       // * 出现在表达式中，是一个解引用符
