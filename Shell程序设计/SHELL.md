...
作为脚本执行的shell程序，没有 alias 别名，没有环境变量 PS1，不读入初始化文件，所有的操作不会影响到父进程。

别名在脚本中默认不开启，所以不要使用别名。
...
通过`ssh`去远程执行长时间运行的命令时，如果网络中断，则终端会收到`HUP`中断信号，从而关闭所有的子进程。所以为了长时间运行程序不受干扰，我们需要：
...
部分命令被设计成 只接受`命令行`参数输入，所以我们需要一个能将 `输入流` 转换为 `命令行参数` 的工具 `xargs`。
